...
check_api_health() {
    ...
    local output code
    local connection_url="http://${HOST}:${PORT}/api/meta/connection_status"
    local periods_url="http://${HOST}:${PORT}/api/meta/period_list"
    local markets_url="http://${HOST}:${PORT}/api/meta/markets"
    local connection_output connection_code
    local periods_output periods_code
    local markets_output markets_code
    ...
    connection_output="$(curl -fsS --max-time "$TIMEOUT_SECONDS" "$connection_url" 2>&1)"
    connection_code=$?
    printf 'url: %s\n' "$connection_url"
    printf 'response: %s\n' "$connection_output"
    if [[ "$connection_code" -eq 0 && "$connection_output" == *'"connected":'* && "$connection_output" != *'"error":'* ]]; then
        status_line "PASS" "Connection status endpoint returned structured status"
    else
        status_line "FAIL" "Connection status endpoint is missing or returned an error payload"
        set_result 2
    fi

    periods_output="$(curl -fsS --max-time "$TIMEOUT_SECONDS" "$periods_url" 2>&1)"
    periods_code=$?
    printf 'url: %s\n' "$periods_url"
    printf 'response: %s\n' "$periods_output"
    if [[ "$periods_code" -eq 0 && "$periods_output" == *'"periods":'* ]]; then
        status_line "PASS" "Period list endpoint returned available periods"
    else
        status_line "FAIL" "Period list endpoint is unavailable"
        set_result 2
    fi

    markets_output="$(curl -fsS --max-time "$TIMEOUT_SECONDS" "$markets_url" 2>&1)"
    markets_code=$?
    printf 'url: %s\n' "$markets_url"
    printf 'response: %s\n' "$markets_output"
    if [[ "$markets_code" -eq 0 && "$markets_output" == *'"markets":'* ]]; then
        status_line "PASS" "Markets endpoint returned available markets"
    else
        status_line "FAIL" "Markets endpoint is unavailable"
        set_result 2
    fi
}
