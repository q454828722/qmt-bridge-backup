...
$PidFile = Join-Path $ProjectRoot "starbridge-quant.pid"
$LegacyPidFile = Join-Path $ProjectRoot "qmt-bridge.pid"
$ServerExe = Join-Path $ProjectRoot ".venv\Scripts\starbridge-server.exe"
$LegacyServerExe = Join-Path $ProjectRoot ".venv\Scripts\qmt-server.exe"
...
function Test-QmtServerProcess {
    ...
    return ($commandLine -like "*starbridge-server.exe*") -or ($commandLine -like "*qmt-server.exe*")
}
...
function Wait-QmtServerListener {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if (-not (Test-QmtServerProcess -ProcessId $ExpectedProcessId)) {
            throw "starbridge-server exited during startup. Check $StderrLog"
        }
...
if (-not (Test-Path $ServerExe)) {
    if (Test-Path $LegacyServerExe) {
        $ServerExe = $LegacyServerExe
    } else {
        throw "starbridge-server.exe not found: $ServerExe"
    }
}
...
if ($listenerPid) {
    $listenerPid | Set-Content -Encoding ascii -NoNewline $PidFile
    Write-Output "StarBridge Quant already listening on port $Port (PID $listenerPid)."
    exit 0
}
...
if (Test-Path $PidFile) {
    ...
    if ([int]::TryParse($oldPidText, [ref]$oldPid) -and (Test-QmtServerProcess -ProcessId $oldPid)) {
        Write-Output "StarBridge Quant process already running (PID $oldPid), waiting for port $Port."
        $listenerPid = Wait-QmtServerListener -ExpectedProcessId $oldPid -ListenPort $Port -TimeoutSeconds $StartupTimeoutSeconds
        if ($listenerPid) {
            Write-Output "StarBridge Quant started listening on port $Port (PID $listenerPid)."
            exit 0
        }
        throw "starbridge-server process $oldPid is running but port $Port is not listening within $StartupTimeoutSeconds seconds."
    }
    Remove-Item $PidFile -Force -ErrorAction SilentlyContinue
}
if ((-not (Test-Path $PidFile)) -and (Test-Path $LegacyPidFile)) {
    Copy-Item $LegacyPidFile $PidFile -Force
}
...
$process.Id | Set-Content -Encoding ascii -NoNewline $LegacyPidFile

$listenerPid = Wait-QmtServerListener -ExpectedProcessId $process.Id -ListenPort $Port -TimeoutSeconds $StartupTimeoutSeconds
if ($listenerPid) {
    Write-Output "StarBridge Quant started on port $Port (PID $listenerPid)."
    exit 0
}

throw "starbridge-server did not start listening on port $Port within $StartupTimeoutSeconds seconds. Check $StderrLog"
