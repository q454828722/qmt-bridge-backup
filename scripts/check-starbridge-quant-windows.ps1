param(
    ...
    [int]$Port = 18888,
    [string]$TaskName = "StarBridge Quant Server",
...
Write-Output "Windows StarBridge Quant check"
...
if ($null -eq $task) {
    ...
    if ($null -ne $info) {
        ...
        $taskOk = ($info.LastTaskResult -eq 0 -or $info.LastTaskResult -eq 267009 -or $info.LastTaskResult -eq 267011)
    }
...
if ($null -eq $listeners) {
    ...
    foreach ($conn in $listeners) {
        ...
        $isQmt = ($commandLine -like "*starbridge-server.exe*") -or ($commandLine -like "*qmt-server.exe*")
        if ($isQmt) {
            $serverFound = $true
        }
        Write-Output ("listener: {0}:{1} pid={2} process={3} bridge_server={4}" -f $conn.LocalAddress, $conn.LocalPort, $conn.OwningProcess, $name, $isQmt)
        if ($path) {
...
if ($serverFound) {
    Write-Status "PASS" "Windows starbridge-server listener is present"
} else {
    Write-Status "FAIL" "Windows starbridge-server listener was not found"
    Set-Result 2
}
...
switch ($overall) {
    0 { Write-Status "PASS" "Windows starbridge-server environment is healthy" }
    1 { Write-Status "WARN" "Windows starbridge-server is usable, but cleanup is recommended" }
    default { Write-Status "FAIL" "Windows starbridge-server environment is not healthy" }
}
