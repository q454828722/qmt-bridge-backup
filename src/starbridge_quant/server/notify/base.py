...
logger = logging.getLogger("starbridge_quant.notify")
...
class NotifierManager:
    ...
    配置方式（通过环境变量）：
    - STARBRIDGE_NOTIFY_BACKENDS: 逗号分隔的后端名称（如 "feishu,webhook"）
    - STARBRIDGE_NOTIFY_EVENT_TYPES: 事件类型白名单（可选）
    - STARBRIDGE_NOTIFY_IGNORE_EVENT_TYPES: 事件类型黑名单（可选）
    """
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not self._backends:
            logger.warning(
                ...
                "(set STARBRIDGE_NOTIFY_BACKENDS)"
            )
...
async def test_notify(request: Request):
    ...
    test_event = {
        "type": "test",
        "data": {"message": "StarBridge Quant notification test"},
    }
